...
extern int acc ;
extern map<int,int> frequency_set;                                          
extern priority_queue<pair<int,int>> maxHeap;    
extern priority_queue<pair<int,int>> Hot;    
extern priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Cold;  
extern priority_queue<pair<int,int>> Eviction_Hot;    
extern priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Eviction_Cold;  
extern vector<int> freq;  
extern vector<int> freq_eviction;  
extern vector<set<int>> potential_check_sets;
extern int num_tag_comp;
extern set<uint64_t> buffer;;
extern map<uint64_t,uint64_t> mpp_time;
extern uint64_t timee;
extern int MAX_SIZE_BUFFER;
extern int cnt;
//extern vector<int> buff;   
//================================================================================
...
class CACHE : public MEMORY {
  public:
    ...
    void handle_fill(),
         ...
         handle_read(),
         handle_prefetch();
    int helper(int set,uint64_t address); // changes here
