...
// declaration by Prashant
// ================================Variable Declaration=================================================
map<int,int> frequency_set;   
priority_queue<pair<int,int>> maxHeap; 
priority_queue<pair<int,int>> Hot; 
priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Cold; 
priority_queue<pair<int,int>> Eviction_Hot; 
priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Eviction_Cold; 
vector<int> freq_eviction(2048,0);
vector<set<int>> potential_check_sets(2048);
vector<int> freq(2048,0);                                 
vector<int> redirect(2048,0);                                 
vector<int> final_set(2048,0);    
set<uint64_t> buffer;
map<uint64_t,uint64_t> mpp_time;
int cnt = 0;
int num_tag_comp = 0;
int ct = 1;
int acc = 0;
...
int x = 0;
uint64_t timee = 0;
vector<bool> indicator(2048,false);

#include <cstdlib> // For rand() function
#include <ctime>   // For srand() function to seed random number generator


//=====================================Function Defination==============================================


bool isfind_hot(int set){
  priority_queue<pair<int,int>> Hot_copy = Hot; 
    while(!Hot_copy.empty()){
        int have = Hot_copy.top().second;
        Hot_copy.pop();
        if(have == set) return true;
    }
    return false;
}
bool isfind_cold(int set){
  priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>> Cold_copy = Cold; 
    while(!Cold_copy.empty()){
        int have = Cold_copy.top().second;
        Cold_copy.pop();
        if(have == set) return true;
    }
    return false;
}
int CACHE:: helper(int set,uint64_t address){

    for(uint32_t way = 0; way < NUM_WAY; way++){
        
        if(block[set][way].tag == address){
          num_tag_comp++;
          return way;
        }
        else{
          if(isfind_cold(get_set(block[set][way].tag))) return way;
        }
    }
     return -1;
}
...
void CACHE::handle_fill()
{
    ...
    // if(p <= 5) cout<<" hexre  "<<current_core_cycle[0]<<endl,p++;
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            ...
            // can be made a buffer here
            // block[set][way].tag store it in a buffer in lru victim
...
void CACHE::operate()
{
    ...
    handle_fill();
//  if(p <= 5) cout<<" hexre  "<<current_core_cycle[0]<<endl,p++;
   // cout<<"ooo_cpu[i].finish_sim_instr" << num_instr<<endl;
    handle_writeback();
...
{ 
    uint32_t set = (address & ((1 << lg2(NUM_SET)) - 1));
      ...
      // logic of redirection here
  
      if(num_instr % 30000 == 0){
     
        // dip the sets into priority queue
         for(int i = 0; i < 2048; i++) redirect[i] = 0;
            while(!Hot.empty()){
                Hot.pop();
            }
            while(!Cold.empty()){
          ...
          priority_queue<pair<int,int>> Hot_copy; 
          priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Cold_copy; 

          for(int i = 0; i < 2048; i++){
              Hot_copy.push({freq[i],i});
          }
          for(int i = 0; i < 2048; i++){
              Cold_copy.push({freq[i],i});
          }
          while(!Hot_copy.empty() && Hot.size() != 1024){
              int fi = Hot_copy.top().first;
              int se = Hot_copy.top().second;
              Hot_copy.pop();
              Hot.push({fi,se});
          }

          while(!Cold_copy.empty() && Cold.size() != 1024){
            int fi = Cold_copy.top().first;
            int se = Cold_copy.top().second;
            Cold_copy.pop();
            Cold.push({fi,se});
          }
      }
      ...
      bool flag = false;
      // check for either full in buffer or not
      if(cache_type == IS_LLC){
         if(buffer.size() == MAX_SIZE_BUFFER) { // if buffer is full clear the buffer 
            buffer.clear(); 
         }
      }

      // here check if it is present in my buffer or not
       if(cache_type == IS_LLC){
        // to find in buffer
      ...
      // int set_Buffer = rand() % NUM_SET;
      // one more way rather to randmised , one can came up with cold set
           int set_Buffer = Cold.top().second;
           int way_Buffer = rand() % NUM_WAY;
            final_set[set] = set_Buffer;
            long long get =  block[set_Buffer][way_Buffer].tag;
            block[set_Buffer][way_Buffer].tag = address;
            potential_check_sets[set].insert(final_set[set]);
            buffer.insert(get);
            buffer.erase(address); // remove previous value
    ...
    // hit
    for (uint32_t way=0; way<NUM_WAY; way++) {
        // tag comprsion is here
        if (block[final_set[set]][way].valid && (block[final_set[set]][way].tag == address)) {
            num_tag_comp++; 
            flag = true;
            break;
        }
      ...
      if(cache_type == IS_LLC){
          ...
          if(isfind_hot(set) && !flag ){
              priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>> > Cold_copy = Cold; 
              while(!Cold_copy.empty()){
                  int get = Cold_copy.top().second;
                  Cold_copy.pop();
                  // // Logic 1: // simple one to one mapping
                  // final_set[set] = get;
                  // break;


                  //Logic 2:  Multi level Cold caches is filled by this approach
                  if(redirect[get] <= 256){ // can be optimised further by checking it is hot or not
                      redirect[get]++;
                      // redirect of set successfull
                      // indicator[get] = true;
                      ind = true;
                      ct++;
                       potential_check_sets[set].insert(get);
                       final_set[set] = get;
...
{
    uint32_t set = (packet->address & ((1 << lg2(NUM_SET)) - 1));
    uint32_t fset = final_set[(packet->address & ((1 << lg2(NUM_SET)) - 1))];
    // we update the set value as per redirected one
    int match_way = -1;
  ...
  if(match_way != -1) return match_way;
      if(cache_type == IS_LLC){
    
          for (uint32_t way=0; way<NUM_WAY; way++) {
            
              if (block[fset][way].valid && (block[fset][way].tag == packet->address)) {
                  num_tag_comp++;
                  match_way = way;
                  break;
              }
  ...
  if(match_way != -1) return match_way;
    if(cache_type == IS_LLC){
      int j = 0;
      for(auto set:potential_check_sets[set]){
          if(j == 50) break;
          j++;
          for (uint32_t way=0; way<NUM_WAY; way++) {
...
void CACHE::update_fill_cycle()
{
    ...
    for (uint32_t i=0; i<MSHR.SIZE; i++) {
        ...
        cout << " event: " << MSHR.entry[i].e
        vent_cycle << " current: " << current_core_cycle[MSHR.entry[i].cpu] << " next: " << MSHR.next_fill_cycle << endl; });
    }
